...
while len(urls) > 0 and len(contain) < maxNumUrl:
    ...
    # #find all nested urls in the page
    # for tag in soup.find_all('a', href = True):

    # if childUrl not in seen
    # #check if the urls are seen or not
    # #if not, add it to the urls queue
    o_text = soup.get_text()
    lower_text = o_text.lower()
    if "covid" in lower_text:
        contain.append(curr_url)
